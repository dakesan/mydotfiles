...
Execute the parser script using the Bash tool:

```bash
python scripts/parse_labench.py <jsonl_path> <entry_id>
```

**Example:**
```bash
python /Users/oodakemac/.claude/skills/la-bench-parser/scripts/parse_labench.py data/private_test_input.jsonl private_test_1
```

### Output Format

The script outputs structured JSON data containing:
